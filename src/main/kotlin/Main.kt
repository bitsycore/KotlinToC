    ...
    */
    ...
    if (vMainCandidate != null) {
        ...
        val vMainC = buildString {
            ...
            appendLine()
            // Register the stdlib exception types the runtime checks throw, so a
            // bounds/null violation becomes a catchable IndexOutOfBoundsException /
            // NullPointerException instead of a hard exit. The C runtime can't name
            // generated TYPE_IDs, hence this generated registration.
            val vExcRegistration = buildString {
                for ((vSlot, vCls) in listOf("ktc_core_exc_oob" to "IndexOutOfBoundsException",
                                             "ktc_core_exc_npe" to "NullPointerException")) {
                    val vCImpl = "ktc_$vCls\$Impl"
                    appendLine("    ktc_core_exc_register(&$vSlot, ${vCImpl}_TYPE_ID, " +
                        "(ktc_Int)sizeof($vCImpl), (ktc_Int)offsetof($vCImpl, message), \"$vCls\");")
                }
            }
            if (vHasArgs) {
