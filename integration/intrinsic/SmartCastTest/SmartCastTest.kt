...
fun main() {
	...
	if (x != null) {
		val doubled = x * 2
		if (doubled != 84) fatalError("FAIL doubled: $doubled")
	...
	val s: String? = "hello"
	if (s != null) {
		if (s.length != 5) fatalError("FAIL length: ${s.length}")
	...
	val ns: String? = null
	if (ns != null) {
		fatalError("FAIL ns should be null")
	} else {
	...
	if (arr is Array<Int>) {
		...
		fatalError("FAIL arr should be Array<Int>")
	}
	if (arr !is Array<Float>) {
		println("arr is not FloatArray")
	} else {
		fatalError("FAIL arr should not be Array<Float>")
	}
	if (arr.size <= 0) fatalError("FAIL arr.size=${arr.size}")
	println("has items: ${arr.size > 0}")

	// Nullable param in function
	val d1 = describeNullable(null)
	if (d1 != -1) fatalError("FAIL describeNullable null: $d1")
	val d2 = describeNullable(50)
	if (d2 != 0) fatalError("FAIL describeNullable 50: $d2")
	val d3 = describeNullable(200)
	if (d3 != 1) fatalError("FAIL describeNullable 200: $d3")
	...
	if (a != null && b != null) {
		val sum = a + b
		if (sum != 30) fatalError("FAIL sum: $sum")
		println("sum: $sum")
	}
	...
	if (v != null) {
		if (v.x != 3.0f) fatalError("FAIL vec.x: ${v.x}")
		if (v.y != 4.0f) fatalError("FAIL vec.y: ${v.y}")
		println("vec: ${v.x}, ${v.y}")
	}

	val nv: Vec2? = null
	if (nv != null) {
		fatalError("FAIL nv should be null")
	}
	...
	if (r1 != -1) fatalError("FAIL r1")
	if (r2 != 0) fatalError("FAIL r2")
	if (r3 != 1) fatalError("FAIL r3")
