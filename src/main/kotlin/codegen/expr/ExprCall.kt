...
internal fun CCodeGen.genCall(e: CallExpr): String {
	...
	if (e.callee is DotExpr) {
		...
		if (vRecvKtc is KtcType.Nullable) {
			...
			if (!hasNullableReceiverExt(vInnerKtc.toInternalStr, e.callee.name) && !vIsIndirectArray && vInnerKtc !is KtcType.Arr) {
	...
	if (vActiveLambda != null) {
		...
		val vResult = when (val vLast = vBody.lastOrNull()) {
			...
			null          -> ""
			else          -> { emitStmt(vLast, currentInd); "" }
			}
	...
	val vFilledArgs   = if (vSig != null) fillDefaults(vArgs, vSig.params, vSig.params.associate { it.name to it.default }) else vArgs
	...
	if (currentClass != null) {
		...
		if (vMethodDecl != null) {
			...
			val vFilled2  = fillDefaults(vArgs, vMethodDecl.params, effectiveDefaults(vMethodDecl, currentClass), vMethodDecl.name, strict = true)
