...
- **Lifetime — frame-bound by default:** the functor struct is stack-local in the defining frame, so a closure must not outlive that frame. Returning a function type from a non-inline function is rejected (**E023**). For thread closures the spawning frame must `join()` before it returns (the context is `alloca`'d, no heap/free).
  - **Heap promotion — `Closure<F>`:** `closure.copyWith(allocator)` heap-promotes a frame-bound functor to a **`Ref<Closure<F>>`** — a type-erased boxed closure of function type `F` (`Closure<(Int) -> Int>`). One heap block holds the fat pointer (`ktc_Closure`) with its captures folded in, so `freeMem(g)` frees it in one call. It's nameable and **escapes**: return it (`fun make(): Ref<Closure<(Int)->Int>> = …`), store it, hold it in a `List<Ref<Closure<F>>>`. Callable like any function — `g(x)` casts the erased invoke to `F`'s signature. Refused for a closure that captured a stack local by reference (`capture(x.asRef())`) — its address would dangle. A **bare** `Closure<F>` is a frame-bound value; returning one from a non-inline function is refused (E023) — return `Ref<Closure<F>>`. (Heterogeneous reassignment / `obj.field(x)` direct-call on a stored closure are the remaining Phase-2 polish.)
