...
fun main(): Int {
	...
	if (squares[0] != 0) fatalError("FAIL squares[0]")
	println("OK: squares[0] = ${squares[0]}")
	if (squares[1] != 1) fatalError("FAIL squares[1]")
	println("OK: squares[1] = ${squares[1]}")
	if (squares[2] != 4) fatalError("FAIL squares[2]")
	println("OK: squares[2] = ${squares[2]}")
	if (squares[3] != 9) fatalError("FAIL squares[3]")
	println("OK: squares[3] = ${squares[3]}")
	if (squares[4] != 16) fatalError("FAIL squares[4]")
	println("OK: squares[4] = ${squares[4]}")

	// ── Array<Float>(n) { init } ──────────────────────────────────────
	val floats = Array<Float>(4) { it.toFloat() * 0.5f }
	if (floats[0] != 0.0f) fatalError("FAIL floats[0]")
	println("OK: floats[0] = ${floats[0]}")
	if (floats[2] != 1.0f) fatalError("FAIL floats[2]")
	...
	if (doubles[0] != 0.0) fatalError("FAIL doubles[0]")
	println("OK: doubles[0] = ${doubles[0]}")
	if (doubles[1] != 6.25) fatalError("FAIL doubles[1]")
	println("OK: doubles[1] = ${doubles[1]}")
	if (doubles[2] != 25.0) fatalError("FAIL doubles[2]")
	...
	if (seq[0] != 0) fatalError("FAIL seq[0]")
	println("OK: seq[0] = ${seq[0]}")
	if (seq[1] != 11) fatalError("FAIL seq[1]")
	println("OK: seq[1] = ${seq[1]}")
	if (seq[2] != 22) fatalError("FAIL seq[2]")
