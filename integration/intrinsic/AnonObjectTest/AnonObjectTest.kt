...
fun main() {
	...
	if (result != 42) error("FAIL greet returned $result")
	...
	printShape(s)
	if (s.area() < 3.13f || s.area() > 3.15f) error("FAIL area: ${s.area()}")
	if (s.name() != "AnonCircle") error("FAIL name: ${s.name()}")
	...
	if (s2.area() != 9.0f) error("FAIL s2 area: ${s2.area()}")
	if (s2.name() != "AnonSquare") error("FAIL s2 name: ${s2.name()}")
	...
	if (directResult != 99) error("FAIL direct greet: $directResult")
