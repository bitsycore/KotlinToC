...
- **Lifetime — frame-bound by default:** the functor struct is stack-local in the defining frame, so a closure must not outlive that frame. Returning a function type from a non-inline function is rejected (**E023**). For thread closures the spawning frame must `join()` before it returns (the context is `alloca`'d, no heap/free).
  - **Heap promotion:** `closure.copyWith(allocator)` copies the functor to the heap and returns a `Ref<Closure_N>` you own and free with `freeMem` — callable directly (`g(x)` → `Closure_N_invoke(g, x)`, the pointer is already `self`). Refused for a closure that captured a stack local by reference (`capture(x.asRef())`) — its address would dangle. (A heap closure can't yet *escape* its defining function — return type / field / generic-arg can't name or infer the anonymous `Closure_N`; full escaping/returned closures and heterogeneous closure storage remain Phase 2, needing closure-type nameability or a type-erased representation.)
