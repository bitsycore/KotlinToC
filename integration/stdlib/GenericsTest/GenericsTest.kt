...
fun main() {
    ...
    if (firstOf(list) != 10) fatalError("FAIL firstOf")

    // Generic class
    val w = Wrapper<String>("hello")
    println("Wrapper: ${w.get()}")
    if (w.get() != "hello") fatalError("FAIL Wrapper initial")
    w.set("world")
    println("Wrapper after set: ${w.get()}")
    if (w.get() != "world") fatalError("FAIL Wrapper set")

    // Pair
    val p = 1 to "one"
    println("Pair: ${p.first} -> ${p.second}")
    if (p.first != 1 || p.second != "one") fatalError("FAIL Pair")

    // Triple
    val t = Triple(10, "hello", true)
    println("Triple: ${t.first}, ${t.second}, ${t.third}")
    if (t.first != 10 || t.second != "hello" || t.third != true) fatalError("FAIL Triple")

    // Triple with type args
    val t2 = Triple<Boolean, Int, String>(false, 42, "world")
    println("Triple2: ${t2.first}, ${t2.second}, ${t2.third}")
    if (t2.first != false || t2.second != 42 || t2.third != "world") fatalError("FAIL Triple2")
    ...
    if (abc.first != null) fatalError("FAIL abc.first should be null")
    if (abc.second != "Hello world") fatalError("FAIL abc.second")

    var abc2: Pair<Int?, String?>? = Pair<Int?, String?>(null, "Hello world")
    println("abc2 not null: ${abc2 != null}")
    if (abc2 == null) fatalError("FAIL abc2 should not be null")
    abc2 = null
    println("abc2 is null: ${abc2 == null}")
    if (abc2 != null) fatalError("FAIL abc2 should be null")
