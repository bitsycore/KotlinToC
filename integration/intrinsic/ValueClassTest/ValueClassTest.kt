...
fun main() {
	...
	val id = UserId(42)
	if (id.raw != 42) fatalError("FAIL UserId raw access")
	...
	if (sum.raw != 42) fatalError("FAIL addIds")
	println("addIds: ok")
	...
	val b = UserId(7)
	val c = UserId(8)
	if (a.raw != b.raw) fatalError("FAIL UserId equality")
	if (a.raw == c.raw) fatalError("FAIL UserId inequality")
	...
	if (mSum.value != 4.0f) fatalError("FAIL Meters sum")
	println("Meters sum: ok")

	// String value class
	val tok = Token("hello")
	if (tok.raw != "hello") fatalError("FAIL Token raw access")
