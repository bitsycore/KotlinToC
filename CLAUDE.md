...
- **Result<T> + runCatching:** `Result.Failure<T>(exception: Throwable)` carries the Throwable (no more Int codes); `runCatching { ... }` captures Success/Failure; extensions: `getOrNull`, `exceptionOrNull`, `getOrDefault`, `getOrThrow` (rethrows the captured exception), `isSuccess`/`isFailure`. `Result<Unit>` works: in value positions (generic slots, fields, params) Unit lowers to the one-byte `ktc_Unit` value (`KTC_UNIT`); plain Unit-returning functions stay C `void`, and Unit-typed arguments are evaluated for side effects with the unit constant passed instead. A `Nothing`-typed lambda binds T as Unit.
