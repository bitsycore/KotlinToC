...
fun main() {
	...
	if (a.hashCode() != b.hashCode()) fatalError("FAIL same value -> same hash")
	println("same value -> same hash: ok")
	if (a.hashCode() == c.hashCode()) fatalError("FAIL diff value -> diff hash")
	...
	if (p1.hashCode() != p2.hashCode()) fatalError("FAIL Point3D same -> same hash")
	println("Point3D same -> same hash: ok")
	if (p1.hashCode() == p3.hashCode()) fatalError("FAIL Point3D reversed -> diff hash")
	...
	if (pl1.hashCode() != pl2.hashCode()) fatalError("FAIL Player same -> same hash")
	println("Player same -> same hash: ok")
	if (pl1.hashCode() == pl3.hashCode()) fatalError("FAIL Player diff name -> diff hash")
	println("Player diff name -> diff hash: ok")
	if (pl1.hashCode() == pl4.hashCode()) fatalError("FAIL Player diff score -> diff hash")
	println("Player diff score -> diff hash: ok")

	// Order of fields matters
	val pair1 = Pair2(1, 2)
	val pair2 = Pair2(2, 1)
	if (pair1.hashCode() == pair2.hashCode()) fatalError("FAIL Pair2 field order matters")
	println("Pair2 field order matters: ok")

	// Same Pair same hash
	val pair3 = Pair2(1, 2)
	if (pair1.hashCode() != pair3.hashCode()) fatalError("FAIL Pair2 same -> same hash")
	...
	if (r1.hashCode() == r2.hashCode()) fatalError("FAIL RegularClass should differ by address")
	println("RegularClass different because diff address: ok")

	// Zero/negative values
	val z1 = Vec2(0.0f, 0.0f)
	val z2 = Vec2(0.0f, 0.0f)
	if (z1.hashCode() != z2.hashCode()) fatalError("FAIL zero vec same hash")
