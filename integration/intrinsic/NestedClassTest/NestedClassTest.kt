...
fun main() {
	...
	if (container.value() != 99) error("FAIL container value=${container.value()}")
	...
	if (nd.label != "score" || nd.value != 100) error("FAIL nested data: $nd")
	println("Vec2.NestedData: $nd")

	// Deeply nested with method
	val deep = A.B.C(99)
	if (deep.v != 99) error("FAIL deeply nested v=${deep.v}")
	if (deep.doubled() != 198) error("FAIL deep method=${deep.doubled()}")
	println("A.B.C: ${deep.v}, doubled=${deep.doubled()}")

	// Tree node
	val node = Tree.Node(10, -1, -1)
	if (node.value != 10) error("FAIL node value=${node.value}")
	node.left = 5
	node.right = 15
	if (node.left != 5 || node.right != 15) error("FAIL node children")
	...
	if (display.area() != 2073600) error("FAIL display area=${display.area()}")
	println("Config.Display: ${display.width}x${display.height} area=${display.area()}")

	val audio = Config.Audio(80, false)
	if (audio.volume != 80 || audio.muted) error("FAIL audio")
	...
	if (i1.x + i2.x + i3.x != 60) error("FAIL sum=${i1.x + i2.x + i3.x}")
