...
internal fun CCodeGen.emitTryReturnCleanup(ind: String, insideMethod: Boolean = false, inDownToMark: Int = 0) {
	...
	for (vCtx in vCtxs) {
		impl.appendLine("$ind/* leaving 'try' — pop its exception frame and run its finally */")
...
internal fun CCodeGen.emitTry(s: TryStmt, ind: String, insideMethod: Boolean) {
	...
	impl.appendLine("$ind/* try/catch — setjmp/longjmp frame (ktc_core_exception.h) */")
...
private fun CCodeGen.emitCatchBinding(inClause: CatchClause, ind: String) {
	...
	if (classes.containsKey(vTypeName)) {
		...
		impl.appendLine("$vInd/* copy the caught $vTypeName off the arena onto this frame */")
		...
		impl.appendLine("$vInd/* copy the caught exception off the arena into a $vTypeName interface value */")
...
internal fun CCodeGen.emitThrow(s: ThrowStmt, ind: String) {
	...
	if (classes.containsKey(vTypeName)) {
		...
		impl.appendLine("$ind/* throw: deep-copy into the TLS exception arena, longjmp to the innermost try */")
	...
	if (interfaces.containsKey(vTypeName)) {
		...
		impl.appendLine("$ind/* throw of an interface value: dispatch on the concrete typeId for the deep copy */")
