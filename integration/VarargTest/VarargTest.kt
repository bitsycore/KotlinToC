...
fun main() {
    // Empty vararg
    val s0 = sum()
    if (s0 != 0) error("FAIL sum(): $s0")
    println("sum() = $s0")

    // Single arg
    val s1 = sum(42)
    if (s1 != 42) error("FAIL sum(42): $s1")
    println("sum(42) = $s1")

    // Multiple args
    val s5 = sum(1, 2, 3, 4, 5)
    if (s5 != 15) error("FAIL sum(1..5): $s5")
    println("sum(1,2,3,4,5) = $s5")

    // Count
    val c3 = count(10, 20, 30)
    if (c3 != 3) error("FAIL count: $c3")
    println("count(3 args) = $c3")

    // Product
    val p = product(2, 3, 4)
    if (p != 24) error("FAIL product: $p")
